                        ...
                        @foreach ($onSaleProducts as $onSaleProduct)
                            <div class="swiper-slide product-card product-card_style3">
                                <div class="pc__img-wrapper">
                                    <a href="{{ route('shop.details',['product_slug'=>$onSaleProduct->slug]) }}">
                                        <img loading="lazy" src="{{ asset('uploads/products/' . $onSaleProduct->image) }}" width="258" height="313"
                                            alt="{{ $onSaleProduct->name }}" class="pc__img">
                                    </a>
                                </div>

                                <div class="pc__info position-relative">
                                    <h6 class="pc__title">
                                        <a href="{{ route('shop.details',['product_slug'=>$onSaleProduct->slug]) }}">{{ $onSaleProduct->name }}</a>
                                    </h6>
                                    <div class="product-card__price d-flex">
                                        <span class="money price text-secondary">
                                            @if($onSaleProduct->price)
                                            <s>${{ $onSaleProduct->cost }}</s><span style="color: red;">${{ $onSaleProduct->price }}</span>
                                         @else
                                           ${{ $onSaleProduct->price }}
                                         @endif
                                        </span>
                                    ...
                                    <div class="anim_appear-bottom position-absolute bottom-0 start-0 d-none d-sm-flex align-items-center bg-body">
                                        ...
                                        <input type="hidden" name="id" value="{{ $onSaleProduct->id }}">
                                        <input type="hidden" name="name" value="{{ $onSaleProduct->name }}">
                                        <input type="hidden" name="quantity" value="1">
                                        <input type="hidden" name="price" value="{{ $onSaleProduct->price == '' ? $onSaleProduct->cost : $onSaleProduct->price }}">
                                        </form>
                                        @endif

                                        <form action="{{  route('shop.details',['product_slug'=>$onSaleProduct->slug]) }}" method="GET">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $onSaleProduct->id }}">
                                        ...
                                        @if(Cart::instance('wishlist')->content()->where('id',$onSaleProduct->id)->count() > 0)
                                        <form action="{{ route('wishlist.remove_from_wishlist', ['rowId' => Cart::instance('wishlist')->content()->where('id', $onSaleProduct->id)->first()->rowId]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{route('wishlist.add_to_wishlist')}}" method="POST" id="wishlist-form">
                                                ...
                                                <input type="hidden" name="name" value="{{ $onSaleProduct->name }}">
                                                <input type="hidden" name="price" value="{{ $onSaleProduct->price == '' ? $onSaleProduct->cost : $onSaleProduct->price }}">
                                                <input type="hidden" name="quantity" value="{{ $onSaleProduct->quantity }}">
