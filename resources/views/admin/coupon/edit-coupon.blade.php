    ...
    <div class="main-content-wrap">
        ...
        <div class="wg-box">
            <form class="form-new-product form-style-1" method="POST" action="{{ route('admin.coupon.update', $coupon->id)}}">
                ...
                @error('code') <span class="alert alert-danger text-center">{{ $message }}</span> @endif
                <fieldset class="category">
                    <div class="body-title">Coupon Type</div>
                    <div class="select flex-grow">
                        <select class="" name="type">
                            <option value="{{ $coupon->type }}">Select</option>
                            <option value="fixed" {{ $coupon->type == 'fixed' ? 'fixed' : ''}}>Fixed</option>
                            <option value="percent" {{ $coupon->type == 'percent' ? 'percent' : ''}}>Percent</option>
                        </select>
                    </div>
                </fieldset>
                @error('type') <span class="alert alert-danger text-center">{{ $message }}</span> @endif
                <fieldset class="name">
                ...
                @error('value') <span class="alert alert-danger text-center">{{ $message }}</span> @endif
                <fieldset class="name">
                ...
                @error('cart_value') <span class="alert alert-danger text-center">{{ $message }}</span> @endif
                <fieldset class="name">
                ...
                @error('expiry_date') <span class="alert alert-danger text-center">{{ $message }}</span> @endif
                <div class="bot">
